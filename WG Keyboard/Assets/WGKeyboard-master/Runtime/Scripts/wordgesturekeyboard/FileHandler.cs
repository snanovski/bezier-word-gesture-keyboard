...
namespace WordGestureKeyboard
{
  public class FileHandler
  {
    ...
    private const string PathToAssets = "Assets/WGKeyboard-master/Assets/";
    private const string PathToLexicon = PathToAssets + "lexicon/lexicon.txt";

    public FileHandler(string layout, string filename)
    {
      this.layout = layout;
      this.filename = filename;
      var wordsInLexicon = GetAllWordsInLexicon();
      _wordsInLexicon = wordsInLexicon.Item1;
      wordRanking = wordsInLexicon.Item2;
    }
    ...
    public void LoadWordGraphs()
    {
      ...
      foreach (var line in File.ReadLines(path))
      {
        ...
        {
          locationPoints.Add(new Vector2(float.Parse(points[i], CultureInfo.InvariantCulture), float.Parse(points[i + 1], CultureInfo.InvariantCulture)));
        }

        for (var i = 0; i < normalized.Length; i += 2)
        {
          normalizedPoints.Add(new Vector2(float.Parse(normalized[i], CultureInfo.InvariantCulture), float.Parse(normalized[i + 1], CultureInfo.InvariantCulture)));
        }

        //Debug.Log(locationPoints.Select(x => x.ToString()).Aggregate((x, y) => x + ", " + y));
        _locationWordPointsDict.Add(splits[0], locationPoints);
        _normalizedWordPointsDict.Add(splits[0], normalizedPoints);
      }
            Debug.Log("Dict Size = " + _locationWordPointsDict.Count);
      isLoading = false;
    }
    ...
    public void LoadLayouts()
    {
      ...
      foreach (var line in File.ReadLines(path).Skip(6))
      {
        ...
        {
          var splits = line.Split("$$");
          if (float.TryParse(splits[1], NumberStyles.Any, CultureInfo.InvariantCulture, out var f))
          {
