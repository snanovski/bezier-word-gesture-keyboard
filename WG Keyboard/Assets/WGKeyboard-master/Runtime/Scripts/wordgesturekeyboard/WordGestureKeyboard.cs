...
namespace WordGestureKeyboard
{
    public class WordGestureKeyboard : MonoBehaviour
    {
        ...
        [Serializable]
        public class GetPointsEvent : UnityEvent<List<Vector2>>
        {
        ...
        public GetPointsEvent getPointsEvent;
        ...
        private FileHandler _fhLines;
        private FileHandler _fhBezierQuadratic;
        private FileHandler _fhBezierCubic;
        ...
        public UserInputHandler getUIH()
        {
            return _uih;
        }
        ...
        private async void Start()
        {
            ...
            _fhLines = new FileHandler(startingLayout, "graph_qwerty_line_20.txt");
            _fhBezierQuadratic = new FileHandler(startingLayout, "graph_qwerty_bezierquadratic_20.txt");
            _fhBezierCubic = new FileHandler(startingLayout, "graph_qwerty_beziercubic_20.txt");
            _fh = _fhLines;
            _fh.LoadLayouts();
            ...
            await Task.Run(() => { _fhLines.LoadWordGraphs(); _fh = _fhLines; });
            await Task.Run(() => { _fhBezierQuadratic.LoadWordGraphs(); });
            await Task.Run(() => { _fhBezierCubic.LoadWordGraphs(); });
        }

        public void setCurrentGraphs(int x)
        {
            switch (x)
            {
                case 2: 
        ...
        private void Update()
        {
            if (_isWriting)
            {
                ...
                if (!_gpc.isCalculatingPreview)
                {
                    ...
                    //Debug.Log("Points = " + pointsList.Select(x => $"({x.x}, {x.y})").Aggregate((x, y) => x + ", " + y));
            ...
            else if (!_uih.GetIsSamplingPoints() && _notEnded)
            {
                ...
                if (pointsList.Count != 0)
                {
                    ...
                    {
                        getPointsEvent.Invoke(pointsList);
                        _gpc.CalcBestWords(pointsList, _fh.GetLocationWordPointsDict(), _fh.GetNormalizedWordPointsDict(),
        ...
        public void EnterOptions(bool b)
        {
            if (b)
            {
                if (!_isOptionsOpen)
        ...
        {
            //Debug.Log($"Word change {text.text}");
            //Debug.Log($"Old change {_lastInputWord}");
            ...
            previewWord.transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<Text>().text = tempWord;
        }
